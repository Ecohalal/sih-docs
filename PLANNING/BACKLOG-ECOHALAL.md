...
- ✅ *(23/jul)* **REESCOPO FEITO — proposta escrita, ❓ aguarda OK do Renato (P1–P5). Nada codado.** Doc: **`halalsphere-docs/PLANNING/EDICAO-ESCOPO-CERTIFICADO-FASE2-REESCOPO-2026-07-23`** (§8). Núcleo: **o backend da F2 já existe** (`PUT /certifications/:id` cobre datas/norma/notas mas **só audita status**; `PUT /certifications/:id/scope` cobre gerais do escopo **sem auditoria nenhuma**; CRUD M2M completo em `/industrial-classification/certifications/:id/categories`, idem sem auditoria) → pacote = **auditoria transacional padrão F1 + modal "Editar dados gerais" + roteiro**; **zero rota nova/regen, zero migration**. **Fronteira A/C por escrito:** `manualEmit` vive em `certificate/*` (A) → módulo `certification/*` + `industrial-classification` (rotas de categorias) viram domínio C (P4); contrato único de contato = **shape de `GET /certifications/:id` não muda** (a hidratação do Emitir-do-escopo, arquivo da A, só lê); **MarketScope 100% fora** (edição de destinos de cert existente = item novo da A, se a FAMBRAS pedir); norma na C-F2 = **dado** (enum + notas), resolução segue kernel/A; snapshot da Fatia 0 garante que nada disso alcança PDF emitido.
