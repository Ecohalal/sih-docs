...
- 🚧 **Importação da planilha `FRIGORÍFICOS DIVERSOS FUNCIONÁRIOS ALI CHAHINE.xlsx`** (`C:\SIH\`, fora de git).
  ...
  - 🧩 **Fase 2 — pessoas** (a fazer): **46 supervisores** viram `SystemUser` (division IN + `assignedPlants` por SIF; precisa senha inicial/bcrypt → decidir disparo de e-mail de 1º acesso, só **2** são `@fambrashalal.com.br`, o resto pessoal) · **54 degoladores** viram `Collaborator` (`type=degolador`) vinculados às plantas. Dados em `scratchpad/ali-chahine.json`.
