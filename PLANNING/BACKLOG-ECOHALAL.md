...
- ✅ *(23/jul)* **REESCOPO + OK do Renato (P1–P5 aprovados, fronteira gravada no §2) — EM IMPLEMENTAÇÃO.** Doc: **`halalsphere-docs/PLANNING/EDICAO-ESCOPO-CERTIFICADO-FASE2-REESCOPO-2026-07-23`** (§8). Núcleo: **o backend da F2 já existe** (`PUT /certifications/:id` cobre datas/norma/notas mas **só audita status**; `PUT /certifications/:id/scope` cobre gerais do escopo **sem auditoria nenhuma**; CRUD M2M completo em `/industrial-classification/certifications/:id/categories`, idem sem auditoria) → pacote = **auditoria transacional padrão F1 + modal "Editar dados gerais" + roteiro**; **zero rota nova/regen, zero migration**. **Decisões (P1–P5 = sim):** observações = `standardNotes` + `scope.description`; sync legados c/ primária; capacidade/turnos/funcionários entram; VOLUNTARY exposto no select. **Fronteira A/C:** `certificate/*`=A · `certification/*`+`certification-scope/*`+categorias do `industrial-classification`=C; contato único = shape de `GET /certifications/:id`; **MarketScope 100% fora** (destinos = A); norma na C-F2 = **dado** (enum+notas), resolução segue kernel/A; snapshot da Fatia 0 garante que nada alcança PDF emitido.
...
- ✅ *(23/jul, EM PROD) pacote de refinamentos:* back `ae10132` · front `6bc283a`.
  ...
  - 🧩 **Resíduo do multi-origem** (baixo): vincular fontes já na criação (hoje só em edição); auto-preencher SIF/faixa na tabela flat p/ produto com >1 origem (o dado já sai no card derivado e no PDF).
