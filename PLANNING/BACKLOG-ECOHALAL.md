...
- 🟡 **F2 — aprovar a CONFECÇÃO do certificado — FATIA A EM PROD (pushada 24/jul, gate LIGADO). ⚠️ muda o fluxo: emissão manual agora cria RASCUNHO → gestor aprova.** back `013c40d0` (schema `f0047fba` + lógica) · front `1ccfbf91`; reconciliado (back `809b1019`, front `fe502092`). API GW: `/certificates/{proxy+}` já cobre (sem regen). 273 testes back + tsc front. Rotas `GET /certificates/drafts`, `POST /:id/{approve,reject}`; tela `/analista/aprovar-certificados` (menu do gestor). Gate: `User.specialtyArea` (default `ambos`) × **novo `Plant.specialtyArea`** (backfill: abatedouro/frigorifico→frigorifico, processamento/laticinio→industrial, resto NULL). Segregação §4 (auto-aprovação auditada). 🔧 **[Renato] (1) briefar FAMBRAS do novo passo emitir→aprovar · (2) validar: emitir → vira rascunho → "Aprovar Certificados" → aprovar → ativo; reprovar c/ motivo.** 🧩 **Fatia B (futura):** PDF de rascunho (marca + QR inválido) · numeração indicativo→definitivo · editar-após-reprovar com re-resolução. Doc: **`halalsphere-docs/ARCHITECTURE/ADR-APROVACAO-CONFECCAO-CERTIFICADO-F2-2026-07-24`** (§8). Núcleo: portão **no nível do DOCUMENTO**, **ortogonal ao Comitê** (que aprova a *certificação*, não a peça) — sem duplicação · aprovador = **gestor da área** (frigorífico→André, industrializados→Fuad) · incide **principalmente na emissão MANUAL** (que hoje cria `ativo` direto) · `Certificate` ganha estado **`rascunho`** antes de `ativo` · número **indicativo no rascunho / definitivo na aprovação** (não queima nº) · **"travar" já feito** pelo snapshot da Fatia 0 · PDF de rascunho com marca + sem QR · **segregação**: aprovador ≠ quem confeccionou, exceção auto-aprovada **auditada** (recomendação Claude, OK Renato). Substrato de auditoria = `CertificationHistory` da C-F2. Toca A e C — coordenar. **É onde converge o antigo "caminho feliz #7"** (o preview antes da oficialização que os operacionais pediram). Fatias A/B/C no ADR §7; pergunta aberta menor = mapeamento divisão→gestor no `User`.
