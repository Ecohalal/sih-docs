...
- 🚧 **Importação da planilha `FRIGORÍFICOS DIVERSOS FUNCIONÁRIOS ALI CHAHINE.xlsx`** (`C:\SIH\`, fora de git).
  ...
  - 🧩 **Fase 2b — supervisores** (a fazer): **46 supervisores**, **verificado que NENHUM já existe** (nem por e-mail nem por nome; sem duplicata por nome). Viram `SystemUser` (division IN + `assignedPlants` por SIF). ❓ **[Renato] Bloqueio:** precisa senha inicial (bcrypt, não dá em SQL puro) → **decidir o disparo de e-mail de 1º acesso** — só **2** são `@fambrashalal.com.br`, os outros 44 são e-mail pessoal (gmail/hotmail). Dados prontos em `scratchpad/fase2-supervisores.json`.
