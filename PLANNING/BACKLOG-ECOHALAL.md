...
- 🎯 **F2 — aprovar a CONFECÇÃO do certificado (rascunho→aprovar→travar) — DESENHO FECHADO com o Renato (24/jul, demanda FAMBRAS). ADR escrito, aguarda início da Fatia A. Zero hash.** Doc: **`halalsphere-docs/ARCHITECTURE/ADR-APROVACAO-CONFECCAO-CERTIFICADO-F2-2026-07-24`** (§8). Núcleo: portão **no nível do DOCUMENTO**, **ortogonal ao Comitê** (que aprova a *certificação*, não a peça) — sem duplicação · aprovador = **gestor da área** (frigorífico→André, industrializados→Fuad) · incide **principalmente na emissão MANUAL** (que hoje cria `ativo` direto) · `Certificate` ganha estado **`rascunho`** antes de `ativo` · número **indicativo no rascunho / definitivo na aprovação** (não queima nº) · **"travar" já feito** pelo snapshot da Fatia 0 · PDF de rascunho com marca + sem QR · **segregação**: aprovador ≠ quem confeccionou, exceção auto-aprovada **auditada** (recomendação Claude, OK Renato). Substrato de auditoria = `CertificationHistory` da C-F2. Toca A e C — coordenar. **É onde converge o antigo "caminho feliz #7"** (o preview antes da oficialização que os operacionais pediram). Fatias A/B/C no ADR §7; pergunta aberta menor = mapeamento divisão→gestor no `User`.
