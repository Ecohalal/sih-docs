...
- 🟡 **F2 — aprovar a CONFECÇÃO do certificado — FATIA A + A.1 (DEPARTAMENTO) EM PROD; curadoria FM 7.8.x EXECUTADA.** ⚠️ muda o fluxo: **emissão manual cria RASCUNHO → gestor da área aprova** (tela `/analista/aprovar-certificados`, menu do gestor). Doc: **`ADR-APROVACAO-CONFECCAO-CERTIFICADO-F2-2026-07-24`** (§8; §8.1 revisado pós-reunião frigorífico).
  ...
  - 🧩 **Fatia A.2 — visibilidade do departamento:** badge nas listas/detalhe · **filtros por depto** (certificações, empresas, grupos — "não faz sentido trazer grupos industrializados", André) · **notificações de vencimento recortadas por depto** (queixa André). 🧩 **Fatia B:** PDF de rascunho (marca + QR inválido) · numeração indicativo→definitivo (⚠️ André: nº definido no comitê deve ser MANTIDO) · editar-após-reprovar. Converge o antigo "caminho feliz #7" (preview pré-oficialização).
...
- 🧩 **Espécie dirige a DT (P0 da reunião):** emitindo aves ou bovino, o sistema seleciona **sempre a DT de bovino** (aves→7.2.1, bovino→7.2.2, 7.3 p/ todos — William). `Plant.species` existe no schema; falta **expor no cadastro + espécie dirigir DT/norma/nº** na emissão. Afeta habilitação/número/qtd de certs (André).
- 🧩 **Trava normativa Malásia × desossa:** planta só-desossa **não pode** emitir Malásia (André+Giovanna; prioridade alta). Par do W15 (aves não pode só-abate/só-desossa — fazem os dois).
- 🧩 **Bug derivação `standard` p/ nacionais (evidência SQL 24/jul):** cert JBS emitido c/ Malásia → card diz "GSO 2055-2" (`standard=GSO_2055_2`) mas escopo/destinos=MS/MY; Indonésia derivou VOLUNTARY. Derivação **inconsistente** (MS→GSO × BPJPH→VOLUNTARY); a tela de emissão mostra o dado certo — o CARD mente. Encosta na decisão Soha §4.3-1.
- 🧩 **SIF + cidade + UF:** (a) lista/tela de empresas (antes da razão social — eles usam nome comum); (b) resultado da busca na emissão mostrar cidade; (c) filtro se perde ao voltar da tela.
...
- 🚧 **Fatia 1** — em curso:
  ...
  - 🧩 **UI "Catálogo de Normas" (fatia futura — decisão Renato 22/jul: "dá mais segurança à FAMBRAS").** Tela admin para editar `certification_standards_by_market` + `restricted_ingredients` **sem DBeaver**. Requisitos do ADR (condição de segurança #4): **admin-gated** · **audit trail por linha** (quem mudou o quê — ISO 17065) · idealmente **propor→aprovar** (conversa com F2 draft→aprovar). Sem isso, edição segue DBeaver-only. Rota nova → **regen API GW no mesmo commit**.
