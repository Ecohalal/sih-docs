...
- 🚧 **Importação da planilha `FRIGORÍFICOS DIVERSOS FUNCIONÁRIOS ALI CHAHINE.xlsx`** (`C:\SIH\`, fora de git).
  ...
  - ✅ *(22/jul, EM PROD) **Fase 2b — supervisores**:* **46 criados** como `SystemUser` (role=supervisor, division=IN, `assignedPlants` por SIF; **0 já existiam**, **0 ficaram sem planta**). **Senha temporária DERIVADA do SIF: `{SIF}@Frambras2026`** (decisão do Renato; grafia "Frambras" com R conforme ele digitou). Login por e-mail da planilha. bcrypt rounds=10. Idempotente por e-mail. Script: `scratchpad/exec-supervisores.js` (**não versionado — não commitar hashes/senhas**). O 46º (YAHA MADOULA / FRIGOL 4150) tinha e-mail sem `@` na planilha; corrigido para `yahyamadoula123@gmail.com` (confirmado pelo Renato) e criado.
