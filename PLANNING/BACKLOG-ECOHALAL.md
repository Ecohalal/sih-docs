...
- 🚧 **Fatia 1** — em curso:
  ...
  - ✅ *(22/jul)* **Fatia 2 — ingredientes restritos** (back `15f77a57` + front `cd4ad539`, LOCAIS, push pendente): **2 tabelas NOVAS** `restricted_ingredients` + `restricted_ingredient_standard_rules` (migration `20260722230000`; seed conchonilha: GSO/UAE rejeitam, BPJPH/MUIS/MS aceitam, OIC **sem regra de propósito** — Soha não falou de Turquia); checkbox "contém carmim (E120)" na emissão → resolve variante "(com conchonilha)" do catálogo DT 7.1 + **aviso W11** (banner) quando norma selecionada rejeita; snapshot congela o flag. Curadoria via DBeaver → futura UI Catálogo. jest 36/36.
